...
clf = OneVsRestClassifier(linear_svc)
clf.fit(train_idf,target)
predicted_text = clf.predict(test_idf)

clf.fit(feature_training,target)
predicted_feature = clf.predict(feature_test)

print ("binary prediction",predicted_text.shape)
predicted_text = mlb.inverse_transform(predicted_text)
predicted_feature = mlb.inverse_transform(predicted_feature)

# print (predicted_text)
print ("len predicted",len(predicted_text))

result = []
for idx,x in enumerate(predicted_text):
    print idx,x
    x = list(x)
    y = list(predicted_feature[idx])
    x = list(set().union(x,y))
    if(len(x) == 0):
        x.append(4483)
    # y = map(int, re.findall(r'\d+', x))
    # print y
...
pd.DataFrame(to_write).to_csv("../input/mixed_data_design.tsv",index=False,sep="\t")
